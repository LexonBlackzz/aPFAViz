...
void RendererES2::render(float clockSec, float totalSec, float fps,
    ...
    // Playback stats are rendered by the Android overlay instead of inside
    // the piano-roll framebuffer. Keeping them out of GL makes the roll itself
    // uncluttered and lets the UI expose richer live metrics without changing
    // engine timing or note rendering.
